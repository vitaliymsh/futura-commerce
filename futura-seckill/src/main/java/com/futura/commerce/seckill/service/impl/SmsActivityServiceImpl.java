...
public class SmsActivityServiceImpl implements SmsActivityService {
    ...
    public CommonResult<Page<ActivityListVO>> getActivityList(Integer pageNum, Integer pageSize, Integer type) {
        ...
        List<ActivityListVO> voList = new ArrayList<>();
        if (type == null) {
            voList.addAll(getSeckillList(1));
            voList.addAll(getCouponList(2));
            voList.addAll(getFollowDiscountList(3));
            voList.addAll(getFullReductionList(4));
        } else {
            switch (type) {
                ...
                default -> voList = getSeckillList(1);
            }
        }
    ...
    public CommonResult<Integer> activityStatus(Long id, Integer status) {
        if (id == null) return CommonResult.failed("Invalid activity ID");
        Optional<SmsActivity> actOpt = smsActivityRepository.findById(id);
        if (actOpt.isPresent()) {
            SmsActivity act = actOpt.get();
            act.setStatus(status);
            smsActivityRepository.save(act);
            return CommonResult.success(status, "Status updated successfully");
        }
        return CommonResult.failed("Activity not found");
    }
    ...
    public CommonResult<Page<ActivitySearchDTO>> activitySearch(Integer pageNum, Integer pageSize, ActivitySearchDTO activitySearchDTO) {
        return CommonResult.success(Page.empty(), "Search completed");
    }

    @Override
    public CommonResult<SmsActivity> createActivity(SmsActivity activity) {
        if (activity == null) {
            return CommonResult.failed("Activity data cannot be null");
        }
        ...
        SmsActivity saved = smsActivityRepository.save(activity);
        return CommonResult.success(saved, "Activity created successfully");
    }
}
